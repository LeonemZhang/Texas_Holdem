...
- 2–10 人标准无限注德州扑克，支持完整行动顺序、最小加注、全押和服务端计时。
- 支持主池、多个边池、平分底池，以及无法均分时的奇数筹码分配。
- Windows 客户端可创建房间、扫描局域网牌桌或使用 IP 直连。
- 邀请链接和二维码可供手机、电脑浏览器直接加入，无需安装玩家客户端。
- 桌面与手机共用同一套响应式牌桌，浏览器玩家可以完整参与准备、行动、筹码交换和统计查看。
- 对局意外中断后可由原房主恢复；普通玩家断线后可回到原身份、座位和筹码状态。
- 支持筹码请求、主动给予、操作记录、对局统计和局内称号。
- 发牌、合法行动、计时、牌型比较和底池结算均由房主服务执行；每名玩家只接收经过权限过滤的牌面快照。
...
本项目以 [GNU General Public License v3.0 only](./LICENSE) 发布。第三方项目和依赖仍分别遵循其自身许可证。
