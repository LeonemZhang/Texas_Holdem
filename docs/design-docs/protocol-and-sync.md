...
存在当前牌局时，快照 `game.handNumber` 是 Host 投影的正整数当前局号：未结算牌局为 `room.completedHands + 1`，已结算牌局为 `room.completedHands`。没有当前牌局时不发送该字段。该字段采用可选兼容形式，以便新客户端读取旧 Host 快照；仅在字段缺失时使用一次 `room.completedHands + 1` 的显示回退，不缓存或自行递增局号。
