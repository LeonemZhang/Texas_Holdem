...
- 创建和管理窗口。
- 主窗口在本地渲染器就绪后先最大化到当前显示器工作区再显示；初始宽高只作为还原或最大化失败时的安全回退，不代表固定运行分辨率。
- 提供受限的桌面 IPC。
- 枚举网络适配器。
- 发送 UDP 房间发现请求。
- 创建房间时启动独立房主服务进程。
...
- 牌桌页面将顶部状态、中间牌桌工作区和底部操作区作为互不侵入的布局区域；工具面板由房间页面统一仲裁，同一时间只展示房主管理、筹码交换或统计中的一个。桌面工具面板只覆盖牌桌工作区；移动工具面板打开时隔离底部下注区和加注弹层，并在关闭后恢复加注草稿。
- 手机高度使用动态视口和安全区约束，长结算、准备和工具内容在自身区域滚动，不依赖设备标称物理分辨率。移动结算折叠状态由展示组件内部管理：右上角独立按钮收起详情，新 `handId` 自动展开，桌面端始终展开。结算准备框顶部将就绪、主动摊牌和暂不参与组织为可换行操作组；主动摊牌只消费既有服务端资格与命令，结算玩家筹码直接渲染当前权威快照值。
- 手机座位队列的视觉顺序与桌面座位坐标分离：桌面位置始终按固定座次计算，手机队列按当前行动者置首并在行动者变化时复位滚动；本人行动摘要只属于移动端视觉层，不产生第二个可交互座位。移动行动者把行动状态和顺位合并为唯一标签，避免重复标识相交。
...
- 桌面牌桌采用约 16:9、最大 80rem 的宽桌布和十人局专属座位密度，底部操作区仍限制为 72rem；低高度还原窗口继续通过按高度收缩桌布和纵向滚动保留安全高度。
...
响应式基准至少覆盖 360×800、393×852、402×874、412×915、427×952、430×932、440×956、448×997，以及浏览器工具栏展开后的 360×640、390×667、393×700、412×732、440×760 可用视口。桌面基准包含 1920×1080 显示器最大化后的实际内容区、当前显示器工作区和 1366×768、1280×800 还原窗口；不得以检测或提示分辨率代替布局适配。
