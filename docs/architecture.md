...
- 本机记录管理通过受限 Electron IPC 请求权威统计；运行中记录从内存运行时投影，其余记录从快照、手牌摘要和统计事实重建。牌型比较只在 poker-core/host 完成，renderer 仅渲染已投影的类别、归属和牌面；牌型记录归约只接受摊牌结算的评估结果，未摊牌获胜的评估牌型不进入全局或个人最大牌型。
