...
fake timers 覆盖 `2s/2s/2s/2s`、每个中间街道快照与持久化、另有合法响应时不启动 Runout、恢复不重放，以及暂停、关闭、销毁清理。
...
已完成；Host 建立单一 Runout timer，按当前权威街道继续，递增 sequence/stateVersion，并在河牌后独立等待 2 秒再结算。
...
产品规格、牌局生命周期设计和协议同步设计现已明确服务端逐街权威快照、`2s/2s/2s/2s` 节奏、暂停/关闭/恢复/销毁行为，以及协议字段与版本不变；`INV-BET-003` 和 `INV-HAND-001` 的 Host 缺口已由运行时测试覆盖。
